...
module.exports = (pool) => {
  router.get("/", checkAuth, function (req, res, next) {
    ...
    if (ckname && name) {
      params.push(
        `CONCAT (users.firstname,' ',users.lastname) LIKE '%${name}%'`
      );
    }
    if (ckposition && role) {
  ...
  router.get("/edit/:userid", checkAuth, function (req, res, next) {
    ...
    pool.query(sql, [userid], (err, result) => {
      ...
      res.render("users/edit", { content: result.rows[0], title: `Edit user`, });
    });
  });
  ...
  router.get("/submitdel/:userid", checkAuth, (req, res) => {
